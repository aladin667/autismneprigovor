...
local ambientConnection, outdoorConnection, fogConnection, playerConnection
...
createCheckbox(section1Content, "Fullbright", false, function(value)
    ...
    if value then
        ...
        -- ambient
        local function niggasploit()
            if fogEnabled then  
                local color = Color3.fromRGB(150, 150, 150)
                sethiddenproperty(lighting, "Ambient", color)
                sethiddenproperty(lighting, "OutdoorAmbient", color)
            end
        end

        ambientConnection = lighting:GetPropertyChangedSignal("Ambient"):Connect(niggasploit)
        outdoorConnection = lighting:GetPropertyChangedSignal("OutdoorAmbient"):Connect(niggasploit)
        niggasploit()
        ...
        if outdoorConnection then outdoorConnection:Disconnect() end
        ...
        if originalSettings.Ambient then
            pcall(function()
                sethiddenproperty(lighting, "Ambient", originalSettings.Ambient)
                sethiddenproperty(lighting, "OutdoorAmbient", originalSettings.OutdoorAmbient)
                sethiddenproperty(lighting, "Technology", originalSettings.Technology)
        ...
        for _, obj in pairs(game:GetDescendants()) do
            if obj:IsA("TextLabel") or obj:IsA("TextBox") then
                if not originalFonts[obj] then
                    originalFonts[obj] = obj.Font
                end
                obj.Font = Enum.Font.Arcade 
            end
        end
    else
        for obj, font in pairs(originalFonts) do
...
local fovSlider = createSlider(section1Content, "Value", 70, 120, defaultFOV, function(value)
    currentFOV = value 
end)
